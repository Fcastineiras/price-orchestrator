...
public class PriceRepositoryTest {
    ...
    public void givenJdbcTemplateThrowExceptionWhenGetPriceByCriteriaThenThrowException() {
        when(jdbcTemplate.queryForObject(any(), any(), any(), any(PriceRowMapper.class))).thenThrow(new RuntimeException());
        final PriceCriteria criteria = new PriceCriteria(
                new PriceToApplyReq(1, 1L, LocalDateTime.now()));
        try {
